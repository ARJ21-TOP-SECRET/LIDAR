...
int main (int argc, char** argv)
{
    ...
    // 读取点云数据
    pcl::io::loadPCDFile<pcl::PointXYZ> ("Cloud_data/02_02.pcd", *cloud_in);
    pcl::io::loadPCDFile<pcl::PointXYZ> ("Cloud_data/03_03.pcd", *cloud_out);
    ...
    // 设置直通滤波器 x轴平行屏幕向左
    PT(cloud_in_filtered, 1, -5.0, 5.0);
    //z轴垂直屏幕向前
    PT(cloud_in_filtered, 3, 7, 13);
    //y轴平行屏幕向上
    PT(cloud_in_filtered, 2, -0.82, -0.35);

    // 设置直通滤波器 x轴平行屏幕向左
    PT(cloud_out_filtered, 1, -5.0, 5.0);
    // z轴垂直屏幕向前
    PT(cloud_out_filtered, 3, 7, 13);
    // y轴平行屏幕向上
    PT(cloud_out_filtered, 2, -0.82, -0.35);
